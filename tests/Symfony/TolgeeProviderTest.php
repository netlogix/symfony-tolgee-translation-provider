...
class TolgeeProviderTest extends ProviderTestCase
{
    ...
    public static function toStringProvider(): iterable
    {
        ...
        yield 'app.tolgee.io' => [
            self::createProvider(self::getHttpClient(), $loader, new NullLogger(), 'en', 'app.tolgee.io'),
            'tolgee://app.tolgee.io'
        ];

        yield 'local.dev' => [
            self::createProvider(self::getHttpClient(), $loader, new NullLogger(), 'en', 'local.dev'),
            'tolgee://local.dev'
        ];

        yield 'example.com:99' => [
            self::createProvider(self::getHttpClient(), $loader, new NullLogger(), 'en', 'example.com:99'),
    ...
    public function testReadForOneLocaleAndOneDomain(
        ...
        $response = function (string $method, string $url, array $options = []) use (
            $locale,
            $domain,
            $responseContent
        ): ResponseInterface {
        ...
        static::assertEquals(
            $expectedTranslatorBag->getCatalogue($locale)->all($domain),
            $translatorBag->getCatalogue($locale)->all($domain)
        );
    }
     ...
     */
    public function testReadForManyLocalesAndManyDomains(
        array $locales,
        array $domains,
        ...
        $response = function (string $method, string $url, array $options = []) use (
            ...
            self::assertEquals($domain, implode(',', $domains));
            self::assertEquals($locale, implode(',', $locales));
            $zip = new ZipArchive();
            $tmpZip = tempnam(sys_get_temp_dir(), 'test_export_') . '.zip';
            $zip->open($tmpZip, ZipArchive::CREATE | ZipArchive::OVERWRITE);
            foreach ($domains as $domain) {
                foreach ($locales as $locale) {
                    $zip->addFromString(
                        sprintf('%s/%s.json', $domain, $locale),
                        json_encode($responseContents[$domain][$locale])
                    );
                }
            }
            $zip->close();
