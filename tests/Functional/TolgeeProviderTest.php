...
class TolgeeProviderTest extends KernelTestCase
{
    ...
    {
        $this->client->setResponseFactory(static function ($method, $url, $options) {
        ...
        static::assertSame($excpect, $translations);
    }
    ...
    public function testWrite(): void
    {
        ...
        $this->client->setResponseFactory(static function ($method, $url, $options) use (&$expectedRequests) {
            ...
            if ($method === 'GET' && strpos($url, '/languages') > 1) {
                ...
                while ('' !== ( $data = $body() )) {
                    if (preg_match('/^Content-Disposition:.*filename="?(?<filename>[^" ]*)/m', $data, $matches)) {
                ...
                $fileNames = array_map(static fn($fn) => str_replace('.json', '', $fn), $fileNames);
                sort($fileNames);
                sort($parts);

                $data = HttpClientFixture::getData(
                    $fixture,
                    implode('.', array_merge(['import'], $parts, $fileNames)),
                    $method
                );
        ...
        foreach ($expectedRequests as $key => $value) {
            static::assertSame(0, $value, $key . ' should be called');
        }
    }
    ...
    public function testDelete()
    {
        ...
        $this->client->setResponseFactory(function ($method, $url, $options) use (&$expectedRequests) {
            ...
            if ($method === 'GET' && strpos($url, '/translations/select-all') > 1) {
                ...
                $this->assertContains(1_000_008_002, $body['ids']);
